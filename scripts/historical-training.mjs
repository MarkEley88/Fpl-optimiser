...
async function main(){
 ...
 const trainedPlayerIds=[...new Set((await Promise.all(SEASONS.map(async season=>{const d=await load(season);return d.rows.map(r=>Number(r.id));}))).flat())].filter(Boolean).sort((a,b)=>a-b);\n const generated=[
  "// Generated by scripts/historical-training.mjs.",
  "// Walk-forward historical learning only; live rules remain enforced separately.",
  "export const HISTORICAL_MODEL="+JSON.stringify({trainedSeasons:SEASONS,weights:W,features:KEYS,historicalPlayerIds:trainedPlayerIds,liveRules:{maxFreeTransfers:5,chipSets:2,chipSplitGameweek:20,oneChipPerGameweek:true,freeHitConsecutive:false}},null,2)+" as const;",
  ""
