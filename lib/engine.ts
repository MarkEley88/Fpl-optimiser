...
function transferIdeas(squad:any[],pool:any[],bank:number,fixtures:any[],gw:number){
  ...
  const rows=all.map(x=>{
    ...
    return {
      ...
      reason:(horizonDelta>=6?"Strong 5-GW upgrade":horizonDelta>=3?"Good 5-GW upgrade":horizonDelta>0.35?"Marginal 5-GW upgrade":"No clear 5-GW upgrade")+"; "+(x.cost>0?"costs £"+x.cost.toFixed(1)+"m":"releases £"+Math.abs(x.cost).toFixed(1)+"m")
    };
  });
...
function buildDecisionPlan(initial:any[],pool:any[],fixtures:any[],startGw:number,bank:number,history:any,entryHistory:any=null){
  ...
  for(let gw=startGw;gw<=endGw;gw++){
    ...
    for(const st of states){
      ...
      // 2) TRANSFER SEQUENCES WITHIN THE SAME GAMEWEEK
      // IMPORTANT: affordability is evaluated against the evolving squad/bank
      // after each transfer, not against the starting squad alone. This means
      // an initially unaffordable player can become affordable after a
      // downgrade in the SAME gameweek.
      //
      // The entire legal transfer universe is screened at every sequence
      // stage. We then keep a beam of the strongest partial sequences so the
      // search remains viable on a serverless function. There is deliberately
      ...
      for(let depth=0;depth<maxTransfers;depth++){
        ...
        for(const path of paths){
          const ftUsed=path.transfers.length;
          ...
          for(const x of legal){
            ...
            seen.add(key);
            const nextSquad=applyTransfer(path.squad,x);
            const nextBank=Number((path.bank-x.cost).toFixed(1));
            expanded.push({
              squad:nextSquad,
              bank:nextBank,
              transfers:[...path.transfers,x],
              cheapDelta:Number((path.cheapDelta+x.delta).toFixed(3)),
              hit:Math.max(0,ftUsed+1-st.ft)*4
            });
          }
        }
      ...
      const scoredPaths=paths.filter(p=>p.transfers.length).map(path=>{
        ...
        .sort((a,b)=>b.exactDelta-a.exactDelta)
        .slice(0,8);

      for(const path of scoredPaths){
        const final=path.transfers[path.transfers.length-1];
        const nf=Math.min(5,Math.max(0,st.ft-path.transfers.length)+1);
        const gain=scoreState(path.squad,fixtures,gw,null,false);
        const action=path.transfers.map(t=>t.in.name+" for "+t.out.player.name).join(" → ");
        next.push({
          ...st,
          squad:path.squad,bank:path.bank,ft:nf,
          recentMoves:[
            ...st.recentMoves,
            ...path.transfers.map(t=>({gw,outId:Number(t.out.player.id),inId:Number(t.in.id)}))
          ].slice(-4),
          total:st.total+gain.points-path.hit,
          steps:[...st.steps,{
            gw,
            action:action+(path.hit?" (-"+path.hit+" points)":""),
            chip:null,bank:path.bank,ft:nf,
            formation:gain.formation,cap:gain.cap,
            projectedGain:Number((gain.points-base.points-path.hit).toFixed(2)),
            transferSequence:path.transfers.map(t=>({
              out:t.out.player.name,in:t.in.name,cost:t.cost
            }))
          }]
        });
      }

      // 3) FUNDING IS NOW PART OF THE GENERAL SEQUENCE SEARCH.
      // A downgrade followed by an upgrade in the same GW is naturally
      // discoverable, and a downgrade retained into a later GW is represented
      // by the resulting bank/squad state. Bank has no standalone value.
...
function buildExactTransferPlan(initial:any[],pool:any[],fixtures:any[],startGw:number,bank:number,history:any,entryHistory:any=null){
  ...
  const enumerateWeekEnds=(base:XState)=>{
    ...
    return [...results.values()].map(path=>{
      ...
      const score=scoreState(path.squad,fixtures,base.gw,null,false).points-hit;
      const next:XState={
        gw:base.gw+1,
        squad:path.squad,
        bank:path.bank,
        freeTransfers:Math.min(5,Math.max(0,base.freeTransfers-path.transfers.length)+1),
        chips:base.chips
      };
      const action:Action={
        gw:base.gw,
        transfers:path.transfers.map(t=>({out:t.out.player.name,in:t.in.name,cost:t.cost})),
        hit
      };
      return{state:next,action,points:score};
    });
  };

  const result=exactSearch<XState,Action>({
    start:initialState,
    endGw:endGw,
    expand:(state)=>{
      if(state.gw>endGw)return[];
      return enumerateWeekEnds(state);
    },
    terminalScore:()=>0,
    upperBound,
    key:stateKey
  ...
  const holdBaseline:number=Array.from({length:endGw-startGw+1},(_,i)=>scoreState(initial,fixtures,startGw+i,null,false).points)
    .reduce((a,b)=>a+b,0);
  ...
  if(!result.actions.length||result.score<=holdBaseline+.05){
    ...
    for(let g=startGw;g<=endGw;g++){
      const base=scoreState(initial,fixtures,g,null);
      steps.push({gw:g,action:"Hold",chip:null,bank:Number(bank||0),ft,formation:base.formation,cap:base.cap,projectedGain:0});
      ft=Math.min(5,ft+1);
    }
...
export async function optimiseSquad(picks:any[],elements:Player[],fixtures:any[],gw:number,bank=0,history:any=null,entryHistory:any=null,includeDecisionPlan=true,fastMode=false,planOnly=false,exactMode=false){
  ...
  if(planOnly){
    if(exactMode){
      const exact=buildExactTransferPlan(current,pool,fixtures,gw,Number(bank||0),history,entryHistory);
      return {pool,current,decisionPlan:exact.steps,decisionPlanDiagnostics:planDiagnostics,exactSearch:exact.search};
    }
